...
$(document).ready(function() {
	$.ajax({
			...
			success: function (data) {
				$('#change-username').val(data.usr_un);
				$('#user-id').val(data.usr_id);
				$('#user-account').html('<span class="glyphicon glyphicon-user"></span> '+
					data.usr_fname+' '+data.usr_lname+'<span class="caret"></span>');
			},
			error: function (){
				alert('Error: L7+');
			}
...
/*show all item's issued*/
function user_item_issued()
{
	$.ajax({
			url: '../data/user_item_issued.php',
			type: 'post',
			// dataType: 'json',
			success: function (data) {
				$('#item-issued').html(data);
			},
			error: function(){
				alert('Error: L91+ user_item_issued');
			}
		});
}
user_item_issued();
/*end show all item's issued*/
...
/*list of issuer's request*/
function show_all_issuers_request()
{
	$.ajax({
			url: '../data/show_all_issuers_request.php',
			type: 'post',
			success: function (data) {
				$('#issuers-request').html(data);
			},
			error: function(){
				alert('Error: L143+ all issuers request');
			}
		});
	
}
show_all_issuers_request();
/* end list of issuer's request*/
